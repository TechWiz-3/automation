...
from spinners import Spinners # type: ignore
...
def log(payload):
    ...
    if get_os == 'darwin' and log_to_system == True:  # MacOS
...
def log_preferences():
    # check if unix users want output logged to system logs
    if get_os == 'darwin':
        global log_to_system
        log = pretty.input("[#2985d6][?][/#2985d6] [purple]Would you like output logged to `system.log` as well? [y/n][/purple]")
        if log.lower() == "y":
            log_to_system = True
            pretty.print("[green]Logging to system.log setup :thumbsup:[/green]")

# Function for accepting a valid network IP address
def enter_ip():
    """Function for entering the network address"""
    netw_addr = pretty.input(
        "[#2985d6][?][#2985d6] [purple]Enter a valid IPv4 address [red]currently only able to accept class C IP addresses with no subnetting[/red][/purple] "
            )
    # initial IP address validity check
    try:
        from spinner_utils import animate
        spinner = Spinners.dots
        frames = spinner.value['frames']
        interval = spinner.value['interval']
        animate(frames, interval, "Checking IP address")
        if valid_ipv4(f"{netw_addr}.1"):  # if ip address is valid
            pretty.print("\n[green]IP address valid[/green]")
    except OSError:  # if ip address not valid
        pretty.print("\nIP address invalid, please re-enter", style = "#e6192f")  # ip address not valid
    ...
    subnet = pretty.input("[purple]Enter subnet mask: [/purple]")
    if octet_count == 1:
        pass
    if octet_count == 2:
        pass
